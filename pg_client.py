...
class PgClient:
    ...
    def insert_sticker_pack(self, name, title, thumb, animated):
        query = "insert into sticker_pack(pack_name,title,thumb,animated,recommended,created_at) values(%s,%s,%s,%s,0, get_unix_timestamp()) ON CONFLICT (pack_name) DO UPDATE set created_at = get_unix_timestamp()"
        try:
            cur = self.pg_cl.cursor()
            cur.execute(query, (name, title, thumb, str(animated)))
            self.pg_cl.commit()
            cur.close()
